...
def geojson_merge_list(input_folder_path, output_folder_path, final):
    ...
    if final == False:
        input_dir = P(f"emd/geojson_files/{input_folder_path}")
    ...
    if final == False:
        ...
        with open(f"emd/compiled_geojson_files/{input_folder_path}_geo.json", "w", encoding = "utf-8") as f:
            json.dump(geojs, f, indent = 2, ensure_ascii = False)
            print(f"Saved data to emd/compiled_geojson_files/{input_folder_path}_geo.json!")
