...
def transform_to_geojson(input_file_path, output_folder_path):
    ...
    sig_cd = input_file["response"]["result"]["featureCollection"]["features"][0]["properties"]["sig_cd"]
    ...
    with open(f"{output_folder_path}/{sig_cd}_geo.json", "w", encoding = "utf-8") as f:
        json.dump(geojs, f, indent = 2, ensure_ascii = False)
        print(f"Saved data to {output_folder_path}/{sig_cd}_geo.json!")
...
def transform_to_geojson_list(input_folder_path, output_folder_path):
    ...
    for file_path in input_dir.glob("*.json"):
        ...
        geojs["features"][0]["properties"]["sig_cd"] = int(geojs["features"][0]["properties"]["sig_cd"]) * 100000

        out_file_path = output_dir / f"{sig_cd}_geo.json"

        with open(out_file_path, "w", encoding = "utf-8") as f:
            json.dump(geojs, f, indent = 2, ensure_ascii = False)
            print(f"Saved data to {sig_cd}!")
